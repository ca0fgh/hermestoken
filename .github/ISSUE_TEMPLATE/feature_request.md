---
name: 功能请求
about: 使用简练详细的语言描述希望加入的新功能
title: ''
labels: enhancement
assignees: ''

---

## 提交前必读（请勿删除本节）

- 文档：https://github.com/ca0fgh/hermestoken
- 使用问题先看或先问：https://deepwiki.com/ca0fgh/hermestoken
- 开启透传后的转发相关反馈不接受 issue；透传模式会直接转发请求，请自行确认上游行为。
- 不接受 coding plan、逆向渠道等技术支持类 issue。
- 警告：删除本模板、删除小节标题或随意清空内容的 issue，可能会被直接关闭；重复恶意提交者可能会被 block。

**您当前的 HermesToken 版本**

请填写，例如：`v1.0.0`

**提交确认**

[//]: # (方框内删除已有的空格，填 x 号)
- [ ] **非重复 issue:** 我已确认目前没有类似 issue。
- [ ] **提交前必读:** 我已完整查看过文档 https://github.com/ca0fgh/hermestoken 和项目 README，确认这不是使用、配置或接入类问题，且现有版本无法满足需求。
- [ ] **模板完整:** 我未删除此模板中的任何引导内容或小节标题，并会按要求完整填写。
- [ ] **维护成本:** 我理解项目维护者精力有限，不遵循模板要求的 issue 可能会被无视或直接关闭。

**功能描述**

**应用场景**
