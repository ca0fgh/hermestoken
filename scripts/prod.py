...
def _download_public_asset(url: str, *, timeout_seconds: int) -> bytes:
    ...
    try:
        ...
        raise LauncherError(
            f"Failed to download public frontend asset: {url}. Next step: verify nginx points to the freshly built `web/dist` and rerun `python3 scripts/prod.py update --domain <public-url>`."
        ) from exc
...
def verify_public_frontend_dist(
    *,
    ...
    if _sha256_bytes(public_index) != _sha256_bytes(local_index):
        raise LauncherError(
            "Public frontend verification failed: `/index.html` does not match the freshly built `web/dist/index.html`. Next step: ensure nginx serves the host `web/dist` generated by the launcher, then rerun the production script."
        )
    ...
    for asset_path in asset_paths:
        ...
        if _sha256_bytes(public_asset) != _sha256_bytes(local_asset_path.read_bytes()):
            raise LauncherError(
                f"Public frontend verification failed: `{asset_path}` does not match the freshly built host dist asset. Next step: update the host `web/dist` through `python3 scripts/prod.py update --domain {public_url.rstrip('/')}` instead of running raw docker compose."
            )
