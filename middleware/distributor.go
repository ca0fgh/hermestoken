				...
				if preferredChannelID, found := service.GetPreferredChannelByAffinity(c, modelRequest.Model, usingGroup); found {
					...
					if err == nil && preferred != nil {
						if preferred.Status != common.ChannelStatusEnabled {
							...
							autoGroups := service.GetUserAutoGroupForUser(common.GetContextKeyInt(c, constant.ContextKeyUserId), userGroup)
...
func getModelRequest(c *gin.Context) (*ModelRequest, bool, error) {
	...
	if strings.Contains(path, "/mj/") {
		...
		} else if relayMode == relayconstant.RelayModeVideoFetchByID {
			shouldSelectChannel = false
		}
		c.Set("relay_mode", relayMode)
...
func SetupContextForSelectedChannel(c *gin.Context, channel *model.Channel, modelName string) *types.HermesTokenError {
	...
	key, index, hermesTokenError := channel.GetNextEnabledKey()
