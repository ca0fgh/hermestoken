...
// 任一步骤失败时原子回滚已完成的步骤。
func (s *BillingSession) preConsume(c *gin.Context, quota int) *types.HermesTokenError {
...
func NewBillingSession(c *gin.Context, relayInfo *relaycommon.RelayInfo, preConsumedQuota int) (*BillingSession, *types.HermesTokenError) {
	if relayInfo == nil {
	...
	// 钱包路径需要先检查用户额度
	tryWallet := func() (*BillingSession, *types.HermesTokenError) {
	...
	trySubscription := func() (*BillingSession, *types.HermesTokenError) {
	...
	default:
		...
		if apiErr != nil {
			if shouldFallbackToWalletAfterSubscriptionError(apiErr) {
				return tryWallet()
			}
			return nil, apiErr
		}
...
func shouldFallbackToWalletAfterSubscriptionError(err *types.HermesTokenError) bool {
	if err == nil || err.GetErrorCode() != types.ErrorCodeInsufficientUserQuota {
		return false
	}
	return !errors.Is(err, model.ErrSubscriptionQuotaInsufficient)
}
