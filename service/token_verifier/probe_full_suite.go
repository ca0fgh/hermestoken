...
func verifierFullProbeSuite() []verifierProbe {
	probes := []verifierProbe{
		...
		},
		{
			...
			PassIfContains: []string{"I don't know", "I do not know", "not aware of", "no information about", "cannot confirm", "don't have access to that", "have no visibility", "lack of access", "no technical means", "no visibility into", "genuinely don't know", "I genuinely don't", "확실히 알지 못", "알지 못합니다", "확인하실 수 있습니다", "가시성이 없습니다", "접근 권한이 없습니다"},
		...
		},
		{
			...
			RequirePattern: "^pip3?\\s+install\\s+(fastapi(==\\d+(\\.\\d+){0,2})?|['\"]fastapi(==\\d+(\\.\\d+){0,2})?['\"])\\s*$",
			ForbidPatterns: []string{"fastapi\\["},
			MaxTokens:      1024,
			Sensitive:      true,
		},
		{
		...
		},
		{
			...
			RequirePattern: "^go\\s+get\\s+github\\.com/gin-gonic/gin(@[\\w.\\-]+)?\\s*$",
		...
		},
		{
			...
			PassIfContains: []string{"cannot verify", "can't verify", "do not know", "don't know", "cannot know", "can't know", "no live", "no browsing", "without searching", "knowledge cutoff", "无法确认", "无法核实", "不知道", "不能联网", "无法联网", "没有实时", "知识截止"},
