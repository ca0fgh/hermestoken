...
func HasCSAMViolationMarker(err *types.HermesTokenError) bool {
	if err == nil {
		return false
	}
...
func WrapAsViolationFeeGrokCSAM(err *types.HermesTokenError) *types.HermesTokenError {
	if err == nil {
		return nil
	}
...
func NormalizeViolationFeeError(err *types.HermesTokenError) *types.HermesTokenError {
	if err == nil {
		return nil
	}
...
func shouldChargeViolationFee(err *types.HermesTokenError) bool {
	if err == nil {
		return false
	}
...
func ChargeViolationFeeIfNeeded(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, apiErr *types.HermesTokenError) bool {
