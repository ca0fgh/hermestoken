  ...
  const loadOverrides = async () => {
    ...
    try {
      ...
      if (!groupsRes.data?.success) {
        showError(groupsRes.data?.message || t('加载失败'));
        return;
      }
      ...
      const allGroups = Array.isArray(groupsRes.data.data)
        ? groupsRes.data.data
        : [];
      const userData = userRes.data.data || {};
      const fallbackGroups = userData.group
        ? [
            {
              group: userData.group,
              effective_total_rate_bps: userData.effective_total_rate_bps,
              has_override: userData.has_override,
              override_rate_bps: userData.override_rate_bps,
            },
          ]
        : [];
