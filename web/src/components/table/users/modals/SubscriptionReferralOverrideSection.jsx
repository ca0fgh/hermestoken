...
const SubscriptionReferralOverrideSection = ({ userId }) => {
  ...
  const validateRow = (row) => {
    ...
    if (!Number.isFinite(percentValue)) {
      return t('指定总返佣率必须为数字');
    }
    if (percentValue < 0 || percentValue > 100) {
      return t('指定总返佣率必须在 0 到 100 之间');
    }
    return '';
  };
    ...
    <Card className='!rounded-2xl shadow-sm border-0' loading={loading}>
      <div className='flex flex-col gap-3'>
        <div className='flex items-center justify-between gap-3'>
          <div>
            <Text className='text-lg font-medium'>{t('邀请人返佣指定')}</Text>
              ...
              return (
                ...
                >
                  ...
                  <div className='mt-2 text-xs text-gray-500'>
                    {`${t('当前授权总返佣率')} ${formatRateBpsPercent(
