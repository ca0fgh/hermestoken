...
test('SubscriptionReferralOverrideSection source uses list-based override editor contracts', () => {
  ...
  assert.match(componentSource, /groupsRes\.data\.data/);
  assert.match(componentSource, /\.filter\(\(row\) => row\.hasOverride\)/);
  assert.doesNotMatch(componentSource, /settingsData\.group_rates/);
  assert.doesNotMatch(componentSource, /settingsData\.total_rate_bps/);
  assert.match(
    componentSource,
    /onChange=\{\(value\)\s*=>\s*updateRow\(row\.id,\s*\{\s*group:\s*value/,
  );
  assert.match(
    componentSource,
