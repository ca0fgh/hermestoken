...
func ApplyTaskModelPricing(modelName string, priceData *types.PriceData) bool {
	...
	seconds := 1.0
	if v, ok := otherRatios["seconds"]; ok && v > 0 {
		seconds = v
	} else if v, ok := otherRatios["duration"]; ok && v > 0 {
		seconds = v
	}

	amount := pricing.PerRequest + pricing.PerSecond*seconds
	multiplier := priceData.GroupRatioInfo.GroupRatio
	if pricing.ShouldIncludeOtherRatios() {
	...
	priceData.Quota = common.QuotaFromFloat(amount * common.QuotaPerUnit * multiplier)
