...
func TextHelper(c *gin.Context, info *relaycommon.RelayInfo) (hermesTokenError *types.HermesTokenError) {
	...
	if info.RelayMode == relayconstant.RelayModeChatCompletions &&
		...
		usage, hermesTokenErr := chatCompletionsViaResponses(c, info, adaptor, request)
		if hermesTokenErr != nil {
			return hermesTokenErr
		}
	...
	usage, hermesTokenErr := adaptor.DoResponse(c, httpResp, info)
	if hermesTokenErr != nil {
		// reset status code 重置状态码
		service.ResetStatusCode(hermesTokenErr, statusCodeMappingStr)
		return hermesTokenErr
