...
- Standard `SELECT ... FOR UPDATE` row locks built with GORM query methods in `model/` MUST use `lockForUpdate(tx)`; callers outside `model/` (for example a controller that opens its own `model.DB.Transaction`) MUST use the exported `model.LockForUpdate(tx)`. Do not use the legacy GORM v1 pattern `tx.Set("gorm:query_option", "FOR UPDATE")`, because GORM v2 silently ignores it and no lock is acquired. Do not duplicate `clause.Locking{Strength: "UPDATE"}` at call sites; the shared helper emits `FOR UPDATE` for MySQL/PostgreSQL and skips it for SQLite, where the syntax is unsupported. `TestRowLocksGoThroughTheSharedHelper` in `model/locking_test.go` fails the build if either form reappears outside that helper. Dialect-specific locking with different semantics (for example, a MySQL next-key/gap lock) may use raw SQL only behind explicit database-type branches with valid fallbacks for every supported database.
- When one transaction locks several rows of the same table in a loop (for example, one `User` row per settlement record), acquire them in ascending primary-key order so concurrent transactions cannot take the same rows in opposite orders and deadlock. Order the driving query (`Order("beneficiary_user_id ASC, id ASC")`) or sort the work list before the loop.
