...
func HydrateActiveSubscriptionQuota(users []*User) error {
	...
	for _, user := range users {
		...
		user.SubscriptionAmountTotal = 0
		user.SubscriptionAmountUsed = 0
		user.SubscriptionQuotaUnlimited = false
	...
	var summaries []userSubscriptionQuotaSummary
	err := DB.Model(&UserSubscription{}).
		Select(`
			user_id,
			COALESCE(SUM(CASE WHEN amount_total > 0 THEN amount_total ELSE 0 END), 0) AS amount_total,
			COALESCE(SUM(amount_used), 0) AS amount_used,
			COALESCE(SUM(CASE WHEN amount_total = 0 THEN 1 ELSE 0 END), 0) AS unlimited_rows
		`).
		Where("user_id IN ? AND status = ? AND end_time > ?", userIDs, "active", GetDBTimestamp()).
		Group("user_id").
	...
	summaryByUserID := make(map[int]userSubscriptionQuotaSummary, len(summaries))
	for _, summary := range summaries {
		summaryByUserID[summary.UserId] = summary
	}
	for _, user := range users {
		if user == nil || user.Id <= 0 {
			continue
		}
		summary, ok := summaryByUserID[user.Id]
		if !ok {
			continue
		}
...
func (user *User) ValidateAndFill() (err error) {
	...
	if username == "" || password == "" {
		return newLocalizedAuthError("用户名或密码为空", ErrUserEmptyCredentials)
	}
	// find by username or email
	DB.Where("username = ? OR email = ?", username, username).First(user)
	if user.Password == "" {
		// An account with no password hash (e.g. OAuth-only) can never log in here.
		return newLocalizedAuthError("用户名或密码错误，或用户已被封禁", ErrInvalidCredentials)
	}
