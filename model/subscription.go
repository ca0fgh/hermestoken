...
func selectEffectiveSubscriptionUpgradeGroup(subscriptionGroup, planGroup string) string {
	subscriptionGroup = strings.TrimSpace(subscriptionGroup)
	planGroup = strings.TrimSpace(planGroup)
...
func getEffectiveSubscriptionUpgradeGroupTx(tx *gorm.DB, sub *UserSubscription) (string, error) {
	if sub == nil {
	...
	if sub.PlanId > 0 {
		...
		if plan != nil {
			planGroup = plan.UpgradeGroup
		}
	}

	return selectEffectiveSubscriptionUpgradeGroup(sub.UpgradeGroup, planGroup), nil
}

func findLatestActiveSubscriptionUpgradeGroupTx(tx *gorm.DB, userId int, now int64, excludeSubscriptionID int) (string, error) {
	if tx == nil {
...
func downgradeUserGroupForSubscriptionTx(tx *gorm.DB, sub *UserSubscription, now int64) (string, error) {
	...
	upgradeGroup, err := getEffectiveSubscriptionUpgradeGroupTx(tx, sub)
	if err != nil {
		return "", err
	}
	if upgradeGroup == "" {
	...
	targetGroup, err := findLatestActiveSubscriptionUpgradeGroupTx(tx, sub.UserId, now, sub.Id)
	if err != nil {
		return "", err
	}
	if targetGroup == "" {
...
func validateSubscriptionOrderPayment(order *SubscriptionOrder, plan *SubscriptionPlan, verification *SubscriptionPaymentVerification) error {
	...
	if verification.ProductID != "" {
		...
		switch strings.TrimSpace(verification.PaymentProvider) {
		case PaymentProviderWaffoPancake:
			expectedProductID = strings.TrimSpace(plan.WaffoPancakeProductId)
...
func completeSubscriptionOrderWithValidation(tradeNo string, verification *SubscriptionPaymentVerification, providerPayload string, expectedPaymentProvider string, actualPaymentMethod string) error {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := ApplySubscriptionReferralOnOrderSuccessTx(tx, &order, plan); err != nil {
			return err
		}
...
func AdminBindSubscription(userId int, planId int, sourceNote string) (string, error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		var lockedPlan SubscriptionPlan
		if err := lockForUpdate(tx).Where("id = ?", plan.Id).First(&lockedPlan).Error; err != nil {
			return err
		}
...
func PurchaseSubscriptionWithBalance(userId int, planId int) error {
	...
	var upgradeGroup string
	err := DB.Transaction(func(tx *gorm.DB) error {
		plan, err := getSubscriptionPlanByIdTx(tx, planId, false)
		if err != nil {
			return err
		}
		if !plan.Enabled {
...
func GetActiveUserSubscriptionUpgradeGroups(userId int) ([]string, error) {
	...
	for _, row := range rows {
		group := selectEffectiveSubscriptionUpgradeGroup(row.SubscriptionUpgradeGroup, row.PlanUpgradeGroup)
		if group == "" {
			continue
		}
...
func buildSubscriptionSummaries(subs []UserSubscription) ([]SubscriptionSummary, error) {
	...
	plansById := make(map[int]SubscriptionSummaryPlan, len(planIds))
	if len(planIds) > 0 {
		...
		for _, plan := range plans {
			plansById[plan.Id] = SubscriptionSummaryPlan{
				Id:    plan.Id,
				Title: plan.Title,
			}
		}
...
func AdminResetUserSubscriptionsByPlan(userId int, planId int, advanceResetTime bool) (*SubscriptionResetResult, error) {
	...
	now := GetDBTimestamp()
	err := DB.Transaction(func(tx *gorm.DB) error {
		// includeDeleted: 软删除的套餐下仍可能有存活订阅,与 ExpireDueSubscriptions 一致。
		plan, err := getSubscriptionPlanByIdTx(tx, planId, true)
		if err != nil {
			return err
		}
	...
	for userId := range userIds {
		...
		err := DB.Transaction(func(tx *gorm.DB) error {
			...
			for i := range dueSubs {
				...
				expiredCount++
				targetGroup, err := downgradeUserGroupForSubscriptionTx(tx, &dueSubs[i], now)
				if err != nil {
					return err
				}
				if targetGroup != "" {
