...
func RequestWaffoPancakePay(c *gin.Context) {
	...
	topUp := &model.TopUp{
		...
		Currency:          "USD",
		ProviderProductID: setting.WaffoPancakeProductID,
...
func WaffoPancakeWebhook(c *gin.Context) {
	...
	if isSubscription {
		...
		verification := &model.SubscriptionPaymentVerification{
			PaymentMethod:   model.PaymentMethodWaffoPancake,
			PaymentProvider: model.PaymentProviderWaffoPancake,
			PaidMoney:       event.Data.Amount,
			PaidCurrency:    event.Data.Currency,
			ProductID:       strings.TrimSpace(event.Data.OrderMetadata[service.WaffoPancakeMetadataProductID]),
		}
		if err := model.CompleteSubscriptionOrderWithValidation(tradeNo, verification, string(bodyBytes)); err != nil {
	...
	if err := model.RechargeWaffoPancake(
		tradeNo,
		event.Data.Amount,
		event.Data.Currency,
		strings.TrimSpace(event.Data.OrderMetadata[service.WaffoPancakeMetadataProductID]),
	); err != nil {
